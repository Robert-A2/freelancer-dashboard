...
| `clients` | `/clients` (list) and `/clients/[name]` (detail) — Client Trust & Risk Center. Top-level keys: `label`, `title`, `subtitle`, `intentDataWarning`, `emptyState`. Sub-namespaces: `summary` (`totalClients`, `current`, `followUp`, `inactive`), `alerts`, `list`, `status` (`current`, `watch`, `risk`, `inactive`, `unidentified`), `dependency`, `trend`. The `detail.*` namespace covers: `back`, `clientSince`, `duration`, `overview`, `pattern`, `revenueTrend`, `dependencyRisk`, `financialContext`, `insights`, `actions` (keyed as "Payment Signals"), `history`, `relationshipHealth`, `reliability` (scores: `excellent`/`good`/`watch`/`risk`/`inactive`), `revenueStory`, `momentum`, `simulator`, `timeline`, `inactiveClient` (`label`/`note`), and four identity keys: `identityUnknown`, `identityUnknownTitle`, `identityUnknownNote`, `identityTimelineNote` (shown when a client's identity cannot be verified from the bank description). |
...
| `insights` | **All ICU MessageFormat templates** for `Insight.key` values — every sentence `buildHistoricalInsights()` and `generateDashboardIntelligence()` can produce. Sub-namespaces include `context`, `comparison`, `snapshot`, `trajectory`, `health`, `intent`, `life`, `forecast`, `opportunity`, `risk`, `notable`. See §6/§7. |
