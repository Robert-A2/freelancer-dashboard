        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            ...
            { key: "transactions", label: t("howBuilt.transactions"), value: coverage.count.toLocaleString(INTL_LOCALES[locale]) },
            {
              ...
              color: forecast?.confidence === "high" ? "text-[#4CC4A4]" : forecast?.confidence === "medium" ? "text-[#D4A254]" : "text-[#E5484D]",
            },
          ].map(item => (
          ...
          <div className="mb-5">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-xs font-bold tabular-nums ${
                forecast.confidence === "high" ? "text-[#4CC4A4]" : forecast.confidence === "medium" ? "text-[#D4A254]" : "text-[#E5484D]"
              }`}>{Math.round(forecast.confidenceScore * 100)}%</span>
            </div>
            <div className="h-2 bg-[#1A3048] rounded-full overflow-hidden mb-3">
              <div className={`h-full rounded-full ${forecast.confidence === "high" ? "bg-[#4CC4A4]" : forecast.confidence === "medium" ? "bg-[#D4A254]" : "bg-[#E5484D]"}`} style={{ width: `${Math.round(forecast.confidenceScore * 100)}%` }} />
            </div>
          </div>
        )}
