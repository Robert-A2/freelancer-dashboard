...
export default function DeleteAccountSection({ email }: { email: string }) {
  ...
  return (
    <>
      <div className="card border-[#E5484D25]">
        <p className="label text-[#E5484D] mb-1">{t("dangerZone")}</p>
        <p className="text-sm text-[#7BA8C4] leading-relaxed mb-4">
          {t("warning")}
        </p>
        <button
          onClick={() => setStep("confirming")}
          className="text-sm font-semibold px-5 py-3 rounded-xl bg-[#E5484D15] hover:bg-[#E5484D25] text-[#E5484D] transition-colors duration-150 min-h-[48px]"
        >
          {t("button")}
        </button>
      </div>
            ...
            {step === "done" ? (
              ...
              <>
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 bg-[#E5484D15] rounded-full flex items-center justify-center text-[#E5484D] text-xl flex-shrink-0">
                    ⚠
                  </div>
                  <div>
                    <h2 className="text-lg font-bold text-[#E8F0F8]">{t("heading")}</h2>
                    <p className="text-xs text-[#E5484D] font-medium">{t("permanentNotice")}</p>
                  </div>
                </div>
                ...
                <ul className="bg-[#1A3048] rounded-xl px-4 py-3 mb-5 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5">
                  {DELETED_ITEM_KEYS.map((key) => (
                    <li key={key} className="text-xs text-[#7BA8C4] flex items-center gap-2">
                      <span className="w-1 h-1 rounded-full bg-[#E5484D] flex-shrink-0" />
                      {t(`items.${key}`)}
                    </li>
                  ))}
                </ul>
                ...
                <label className="block text-sm text-[#A8C6E0] mb-2">
                  {t.rich("confirmPrompt", {
                    word: CONFIRM_WORD,
                    b: (chunks) => <span className="font-mono font-semibold text-[#E5484D]">{chunks}</span>,
                  })}
                </label>
                <input
                  type="text"
                ...
                {error && (
                  <p className="text-sm text-[#E5484D] mb-4">{error}</p>
                )}
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 order-1 sm:order-2 font-semibold px-6 py-3 rounded-xl transition-colors duration-150 min-h-[48px] bg-[#E5484D] hover:bg-[#C75F5F] text-[#0D1B2B] disabled:opacity-30 disabled:cursor-not-allowed"
                  >
