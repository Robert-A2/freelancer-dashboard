...
export default function Navbar() {
  ...
  return (
    <>
      {/* ── Top bar ──────────────────────────────────────────────────────── */}
      <nav className="sticky top-0 z-50 border-b border-[#1E293B] bg-[#0A1020]">
        <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-14">

          <div className="flex items-center gap-6">
            <span className="font-bold text-[#F8FAFC] text-sm tracking-tight">
              Freelancer OS
            </span>

            {/* Desktop nav — hidden on mobile since bottom nav handles it */}
          ...
          <button
            ...
            className="text-xs text-[#94A3B8] hover:text-[#F8FAFC] transition-colors px-3 py-2 min-h-[44px]"
          >
            Sign out
          </button>
        </div>
      ...
      {/* Replaces the hamburger drawer — cleaner one-thumb navigation */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#0A1020] border-t border-[#1E293B]">
            ...
            return (
              <Link
                ...
                className={`relative flex flex-col items-center justify-center gap-1 flex-1 py-3 min-h-[56px] transition-colors ${
                  active ? "text-[#14B8A6]" : "text-[#64748B] hover:text-[#94A3B8]"
                }`}
              >
                {/* Active indicator line at the top of each tab */}
                {active && (
                  <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-[#14B8A6] rounded-full" />
                )}
                <Icon />
