    ...
    <div className="card">
      <div className="divide-y divide-[#E8EAE5]">
        {visible.map((tx) => (
          <div key={tx.id} className="flex items-center justify-between py-3 gap-3">
            ...
            <span className={`text-sm font-semibold whitespace-nowrap flex-shrink-0 ${TYPE_COLORS[tx.transactionType] ?? "text-[#6B7280]"}`}>
      ...
      {!expanded && remaining > 0 && (
        <button
          ...
          className="mt-4 w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-[#F7F8F5] hover:bg-[#ECEEE9] text-sm text-[#4B5563] font-medium transition-colors"
        >
